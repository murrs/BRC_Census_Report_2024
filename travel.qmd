...
ggplot(pointOfEntry[pointOfEntry$year == 2024,], aes(x = labels, y = est)) +
  geom_bar(stat = "identity", fill = "#EA008B") +
  scale_y_continuous(labels = percent) +
...
ggplot(delayDays, aes(x = daysDelayed, weight = cweights)) +
  ...
  scale_y_continuous(breaks = c(0, 1209.5, 2419, 3628.5, 5838),
...
ggplot(pointOfDeparture[pointOfDeparture$year == 2024,], 
       aes(x = labels, y = est)) +
  geom_bar(stat = "identity", fill = "#EA008B") +
  scale_y_continuous(labels = percent) +
...
ggplot(towing[towing$year == 2024,], aes(x = labels, y = est)) +
  geom_bar(stat = "identity", fill = "#EA008B") +
  scale_y_continuous(labels = percent) +
  labs(x = "Trailer type", y = "Burning Man participants in 2024 (%)",
       title = "Was your vehicle towing a trailer?") +
  theme_bw() +
  coord_flip()
...
ggplot(vehicleFuelType[vehicleFuelType$year == 2024,], aes(x = labels, y = est)) +
  geom_bar(stat = "identity", fill = "#EA008B") +
  scale_y_continuous(labels = percent) +
  labs(x = "Fuel type", y = "Burning Man participants in 2024 (%)",
       title = "What kind of fuel did your vehicle run on?") +
  theme_bw() +
  coord_flip()
...
### Vehicle MPG on the way to Black Rock City
...
ggplot(mpg[mpg$year == 2024,], aes(x = mpg, weight = weights)) +
  ...
  geom_segment(x = mpg.means$mpg[mpg.means$year == 2024],
               ...
               y = 0, yend = 0.0431, color = "#4C4C4C",
             linewidth = 1.1) +
  annotate("text", x = 37, y = 0.043, label = "Average miles per gallon to \nBlack Rock City (19.0 MPG)") +
  theme_bw(13) +
...
```{r}
nvStops <- data.frame(text = c(census24$nevadaStops))
nvStopsTokens <- tidytext::unnest_tokens(nvStops,
                                         output = tkn,
                                         input = text,
                                         token = "ptb")
nvStopsTokens$tkn[grepl("(las)|(vegas)", nvStopsTokens$tkn)] <- "las vegas"
nvStopsTokens$tkn[nvStopsTokens$tkn == ","] <- NA
nvStopsTokens$tkn[nvStopsTokens$tkn == "and"] <- NA
nvStopsTable <- table(nvStopsTokens$tkn)[order(table(nvStopsTokens$tkn),
                                              decreasing = TRUE)]
nvStopsFreq <- data.frame(text = attr(nvStopsTable, "dimnames")[[1]],
                         freq = as.numeric(nvStopsTable))
wordcloud2(nvStopsFreq[1:20,], color = discreteColors)
```
