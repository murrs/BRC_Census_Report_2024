...
usa_dotdensity <- ggplot() +
  ...
  geom_sf(data = brc_5070, shape = 8, size = 1, color = discreteColors[4], show.legend = FALSE) +
...
CaNV_dotdensity <- ggplot() +
  geom_sf(data = usa_shifted, fill = "gray92", color = "white", size = 0.1) +
  geom_sf(data = density_dots_us_where, aes(color = symbol_group), size = 0.01, alpha = 0.7, show.legend = FALSE) +
  geom_sf(data = brc_5070, shape = 8, size = 2, color = discreteColors[4], show.legend = FALSE) +
  scale_color_manual(values = discreteColors[7:9]) +
  coord_sf(
    crs = 5070,
...
# Position inset zoomed in and on top of west coast 
final_dotdensity <- ggdraw() +
  draw_plot(usa_dotdensity) +
  draw_plot(CaNV_dotdensity, x = 0.1, y = 0.25, width = 0.3, height = 0.5)
print(final_dotdensity)
