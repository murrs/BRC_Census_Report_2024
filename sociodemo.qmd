...
```{r}
ggplot(census24, aes(x = age, y = after_stat(density), weight = weights)) +
...
ageQuantiles <- data.frame(
                      svyquantile(~age, design24, props)[[1]][,1],
...
```{r}
ethnoracial22to24 <- makePlotData("Which category best describes your race?",
                              varNameTable = varNameTable,
                              designs = list(design22, design23, design24),
                              years = 2022:2024,
                              levels = rep(list(TRUE), times = 3),
                              labels = c("Asian",
                                         "Native American",
                                         "Hispanic/Latino",
...
ggplot(ethnoracial22to24[ethnoracial22to24$year == 2024,], aes(x = labels, y = est)) +
...
tableDat <- makeTableData(rbind(ethnoracial22to24, ethnoracial17to19, ethnoracial13to16))
...
# tableDat |>
#   kbl(col.names = c(as.character(2013:2019), "2022^c^", as.character(2024)), 
#       row.names = TRUE) |>
#   kable_paper("striped") |>
