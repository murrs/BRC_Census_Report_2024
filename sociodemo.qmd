  ...
  mutate(
    line = st_sfc(
      st_linestring(matrix(c(
        st_coordinates(brc_man)[1, 1] + 0.6 * (st_coordinates(geometry)[1] - st_coordinates(brc_man)[1, 1]),
        st_coordinates(brc_man)[1, 2] + 0.6 * (st_coordinates(geometry)[2] - st_coordinates(brc_man)[1, 2]),
...
brc_map +
  ...
  scale_size_continuous(range = c(1, 5), name = "Number of Responses") +
  geom_sf_text(data = intersection_summary, aes(label = num_matches), 
               color = "white", size = 1, check_overlap = FALSE) +
