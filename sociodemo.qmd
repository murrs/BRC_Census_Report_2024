...
zctas <- zctas(cb = TRUE, year = 2020, class = "sf") |>
  select(ZCTA = ZCTA5CE20) |>
  shift_geometry() |>              # shift AK, HI, PR, and other territories
  st_transform(5070)

# Load and shift all states/territories to use as outlines
...
count_tbls   <- map(census_list, tally_zips) |>
                set_names(census_list)

US_ZCTAs     <- merge_tallies(zctas, count_tbls)
...
dots_sf <- map_dfr(
  ...
  function(col) {
    purrr::map2_dfr(
      US_ZCTAs[[col]],
      sf::st_geometry(US_ZCTAs),
      \(cnt, geom) {
        if (cnt > 0) {
          pts <- sf::st_sample(geom, size = cnt, exact = FALSE)
          if (length(pts))
            sf::st_as_sf(data.frame(year = col), geometry = pts, crs = st_crs(US_ZCTAs))
        }
      }
    )
  }
