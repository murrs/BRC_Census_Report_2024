...
library(remotes)  # so we can use a shifted version of the Albers projection
library(tidytext)
library(wordcloud2)
...
# Initially just including the last three census
# because they have an explicit question about zip
census_list <- paste0("census", substr(2022:2024, 3, 4)) 

# Create one set of tally tables (counts + weights in one)
stat_tbls <- map(census_list, tally_zip_stats) |> set_names(census_list)

# Merge onto ZCTA data
US_ZCTAs <- merge_tallies(usa_zcta, stat_tbls)

# Dot‑density map
set.seed(123)        # reproducible dot sampling
num_dots_per_wgt = 10 # number of dots per 1 weighted value
...
dots_sf <- map_dfr(
  ...
  function(col) {
    purrr::pmap_dfr(
      list(
        wgt   = US_ZCTAs[[paste0(col, "_wgt")]],
        geom  = st_geometry(US_ZCTAs),
        zcta  = US_ZCTAs$ZCTA,
        state = US_ZCTAs$state_abbr
      ),
      \(wgt, geom, zcta, state) {
        if (wgt > 0) {
          pts <- st_sample(geom, size = round(wgt*num_dots_per_wgt), exact = FALSE)
          if (length(pts)) {
            st_as_sf(
              data.frame(
                year        = col,
...
# Still have to filter out a few territories :(
conus_bbox <- usa_shifted %>%
  filter(!STUSPS %in% c("GU", "VI", "MP", "AS")) %>%
  st_bbox()

# Small multiple dot density maps
ggplot() +
  geom_sf(data = usa_shifted, fill = "gray90", color = "white", size = 0.1) +
  geom_sf(data = dots_sf, aes(color = year), size = 0.2, alpha = 0.7) +
  coord_sf(
    crs = 5070,
    xlim = c(conus_bbox["xmin"], conus_bbox["xmax"]),
    ylim = c(conus_bbox["ymin"], conus_bbox["ymax"])
  ) +
  scale_color_manual(values = discreteColors[1:3], name = "Year") +
  facet_wrap(~ year) +
  theme_minimal() +
  theme(
...
# All US states / select territories, only 2024
ggplot() +
  geom_sf(data = usa_shifted, fill = "gray90", color = "white", size = 0.1) +
  geom_sf(data = filter(dots_sf, year == "census24"), color = discreteColors[1], size = 0.05, alpha = 0.5) +
  coord_sf(
    crs = 5070,
    xlim = c(conus_bbox["xmin"], conus_bbox["xmax"]),
    ylim = c(conus_bbox["ymin"], conus_bbox["ymax"])
...
ggplot() +
  geom_sf(data = filter(usa_shifted, STUSPS %in% c("CA", "NV")), fill = "gray90", color = "white", size = 0.1) +
  geom_sf(data = filter(dots_sf, year == "census24", state_abbr %in% c("CA", "NV")), 
          color = discreteColors[1], size = 0.1, alpha = 0.5) +
  coord_sf(
    crs = 5070,
    xlim = c(conus_bbox["xmin"], conus_bbox["xmax"]),
    ylim = c(conus_bbox["ymin"], conus_bbox["ymax"])
...
# Try combining all three years and tallying by zip
US_ZCTAs_combined <- US_ZCTAs %>%
  rowwise() %>%
  mutate(total_count = sum(c_across(all_of(census_list)), na.rm = TRUE)) %>%
  ungroup()

# Try just looking at 2024 
US_ZCTAs_24 <- US_ZCTAs %>%
  select(ZCTA,census24)

# Distribution very heavily skewed toward just a few zips with more than 1 or 2... 
counts <- US_ZCTAs_24$census24
breaks_vals <- unique(quantile(counts[counts > 0], probs = seq(0, 1, 0.25), na.rm = TRUE))

# Choropleth of zip codes
ggplot(US_ZCTAs_24) +
  geom_sf(data = usa_shifted, fill = "gray90", color = "white", size = 0.3) +
  geom_sf(aes(fill = census24), color = NA) +
  coord_sf(
    crs = 5070,
    xlim = c(conus_bbox["xmin"], conus_bbox["xmax"]),
    ylim = c(conus_bbox["ymin"], conus_bbox["ymax"])
  ...
  scale_fill_gradientn(
    colors = rev(gradientColors(3)),  # reverse the color ramp
    trans = "sqrt",
    breaks = breaks_vals,
    labels = scales::comma_format()(breaks_vals),
    na.value = "white",
    name = "Count (2024)"
  ) +
  theme_minimal() +
