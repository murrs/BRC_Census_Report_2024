...
library(geomtextpath)  # <-- needed for labeling the maps along radials
...
# Pull in all the data for labeling
...
# Create lines pointing toward the center, but not all the way
# Step 1: Create the line geometries and line_id
radial_lines <- brc_radial_label_lines |>
  filter(street2 == "Esplanade") |>
  rowwise() |>
  mutate(
    ...
    interp_x = center_x + 0.6 * (target_x - center_x),
    interp_y = center_y + 0.6 * (target_y - center_y),
    line = st_sfc(
      st_linestring(matrix(c(interp_x, interp_y, target_x, target_y), ncol = 2, byrow = TRUE)),
...
brc_street_label_points <- brc_street_label_points |>
  mutate(
    ...
    y = coords[, 2],
    nudge_x = case_when(
      street1 == "2:00" & street2 == "Esp" ~ 0.0003,
      street1 == "2:00" ~ 0.0001,
      street2 == "Esp" ~ 0.0014,
      TRUE ~ 0.0010
    ),
...
ggplot(reside, aes(x = year, y = est, color = labels)) +
  ...
  scale_x_continuous(breaks = c(2013:2019, 2022:2024),
                     labels = c(2013:2019, 2022:2024)) +
  scale_y_continuous(labels = percent, limits = c(0, NA)) +
  scale_color_manual(values = discreteColors[c(7:9,1:2)]) +
  labs(x = "", y = "Burning Man participants (%)",
...
conus_bbox_minus_some_terr <- usa_shifted %>%
...
final_dotdensity <- ggplot() +
  ...
  geom_sf(data = brc_5070, shape = 8, size = 1.5, color = discreteColors[4], show.legend = FALSE) +
  scale_color_manual(values = discreteColors[7:9]) +
...
final_dotdensity +
  labs(
    title = "Where do you usually reside (within USA)",
    subtitle = "1 dot ≈ 5 Burners"
  ) +
  coord_sf(
    crs = 5070,
    xlim = c(conus_bbox_minus_some_terr["xmin"], conus_bbox_minus_some_terr["xmax"]),
    ylim = c(conus_bbox_minus_some_terr["ymin"], conus_bbox_minus_some_terr["ymax"])
  )
...
final_dotdensity +
  labs(
    title = "Where do you usually reside (California & Nevada)",
    subtitle = "1 dot ≈ 5 Burners"
  ) +
  coord_sf(
    crs = 5070,
...
```{r}
ppGroups <- list(`Democratic Party` = c("Democratic",
                                             "Democratic Party"), 
                      `Green Party` = c("Green", "Green Party"),
                      `Libertarian Party` = c("Libertarian",
                                              "Libertarian Party"), 
                      `None or unaffiliated` = c("None",
                                                 "None or unaffiliated",
                                                 "None, I am nonpartisan"),
                      `Other US Party` = c("Other",
                                           "Other US Party"), 
                      `Republican Party` = c("Republican",
                                             "Republican Party"))
